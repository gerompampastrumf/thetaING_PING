...
#define dinf _p[6]
#define dinf_columnindex 6
#define tauc _p[7]
#define tauc_columnindex 7
#define taud _p[8]
#define taud_columnindex 8
#define Dc _p[9]
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 nca /home/dimitrios/TFM/CA1model/mechanisms/nca.mod\n");
...
/*CVODE*/
 ...
 static int states (double* _p, Datum* _ppvar, Datum* _thread, NrnThread* _nt) { {
   rates ( _threadargscomma_ v ) ;
    ...
    d = d + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / taud)))*(- ( ( ( dinf ) ) / taud ) / ( ( ( ( - 1.0 ) ) ) / taud ) - d) ;
   }
  return 0;
}
...
static int  rates ( _threadargsprotocomma_ double _lv ) {
 ...
 _lalpha = - 0.19 * vtrap ( _threadargscomma_ _lv - 19.88 , - 10.0 ) ;
   ...
   tauc = 1.0 / ( _lalpha + _lbeta ) ;
   cinf = _lalpha / ( _lalpha + _lbeta ) ;
   _lalpha = 0.00016 / exp ( - _lv / 48.4 ) ;
   _lbeta = 1.0 / ( exp ( ( - _lv + 39.0 ) / 10.0 ) + 1.0 ) ;
   taud = 1.0 / ( _lalpha + _lbeta ) ;
   dinf = _lalpha / ( _lalpha + _lbeta ) ;
    return 0; }
...
#if NMODL_TEXT
static const char* nmodl_filename = "/home/dimitrios/TFM/CA1model/mechanisms/nca.mod";
static const char* nmodl_file_text = 
