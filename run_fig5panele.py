...
bas_pyr = [0.0,0.05,0.1,0.3,0.5][2]
pyr_bas = [0.0,10,20,60][input2]

n_ec_neurons=500
noise_bas = 4.0
noise_bas_neg = [1.2,1.5,2.0,5.0,10.0][-1]
ec_bas = [0.5,0.4166,0.332,0.0][input2]
ec_pyr=[0.0,0.03,0.04,0.05,0.06][2]
pyr_gaba_noise=[2.0,5.0,10.0,50.0,100.0][2]
pyr_noise = [1.75,2.1,2.2,2.3,2.4][2]
i_pyr = 0.0
isi_noise = 1.0
isi_noiseDG = [1,5,10,20,50][1]
bas_pyr2=[1.0,2.0,5.0,10.0,20.0][2]
theta_delay = [0,10,20,30,40][2]
shutil.copy(inputs_folder+"/external_inputs_DGnoise"+str(isi_noiseDG)+".lzma",save_folder+"/external_inputs/external_inputs_DGnoise.lzma")
...
weights_inputs_neurons["theta_genIdentical_to_pyr_ca3"]= [[1.0e-3*ec_pyr,1.0e-3*0.1*ec_pyr]]
weights_inputs_neurons["theta_genIdentical_to_pyr_ca3"]= [[1.0e-3*ec_bas,1.0e-3*0.1*ec_bas]]
delay_inputs_neurons["theta_genIdentical_to_pyr_ca3"] = [theta_delay,0.2]
delay_inputs_neurons["theta_genIdentical_to_pyr_ca3"] = [20.0+1.0,0.2]
syn_inputs_neurons["theta_genIdentical_to_pyr_ca3"]    = [["Adend3AMPA_ec2180","Adend3NMDA_ec2180"]]
syn_inputs_neurons["theta_genIdentical_to_pyr_ca3"]    = [["somaAMPA_ec2180","somaNMDA_ec2180"]]
nsyns_inputs_neurons["theta_genIdentical_to_pyr_ca3"]  = [n_ec_neurons]
nsyns_inputs_neurons["theta_genIdentical_to_pyr_ca3"]  = [n_ec_neurons]

weights_inputs_neurons["DGnoise"+str(isi_noiseDG)+"_to_pyr_ca3"]= [[1e-05*isi_noise*isi_noiseDG*2.0,1e-05*isi_noise*isi_noiseDG*2.0/10.0]]
delay_inputs_neurons["DGnoise"+str(isi_noiseDG)+"_to_pyr_ca3"]  = [0.0,0.0]
syn_inputs_neurons["DGnoise"+str(isi_noiseDG)+"_to_pyr_ca3"]    = [["Adend1AMPA_dgreg","Adend1NMDA_dgreg"]]
nsyns_inputs_neurons["DGnoise"+str(isi_noiseDG)+"_to_pyr_ca3"]  = [1]
