...
thres = [0.01,0.005,0.05][int(input3)]
for input1 in range(3):
    ...
    for ind,pos in enumerate(pos_vals):
        ...
        if(ts_choise=="lfp"):
            ...
            ts_choise = ratesnam
            df_pos = df.copy()
        ftheta,fgamma,psi_tmp,nu_tmp= get_avg_cfd(fgamma_lims,ftheta_lims,fs,df_pos,ts_choise,surrogate_test=surrogate_test,nperseg=nperseg,testing=testing,flag_z=flag_z,thres=thres)
    ...
    if(surrogate_test):
        title = "CFD"+str([2,4,5][int(input3)])+"_fft"+str(nperseg)+"_z"+str(flag_z)+"_sign"+testing+"_"+ts_choise+"_"+area
    else:
